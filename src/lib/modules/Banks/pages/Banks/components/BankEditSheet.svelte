<script lang="ts">
    ...
    import { useBankEditForm } from '../hooks/useBankForm.svelte'
    ...
    let { bank, onClose }: Props = $props()

    const ctrl = untrack(() => useBankEditForm(bank, onClose))
</script>

<BottomSheet open title="Editar cuenta bancaria" {onClose}>
